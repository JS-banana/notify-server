...
天气数据API接口：<https://www.tianqiapi.com/> 不再免费提供使用了，新用户注册用户可免费使用2000次

非收费接口也有，不过目前看来功能有限，因此，接下来会考虑其他接口（免费接口很容易这样用着用着就不行了😿）
...
如果按照本项目功能配置使用，注册完成后，直接添加需要的环境变量可以直接测试使用，当然你也可以自行开发定制。
